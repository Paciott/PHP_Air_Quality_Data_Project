...
// acquire city from GET parameter
$city = null;
if (!empty($_GET['city'])) {
    $city = $_GET['city'];
}

// extrapolate file name from json file thanks to city parameter
$filename = null;
$cityInformation = [];
if (!empty($city)) {
    $cities = json_decode(
...
// get contents from filename, we are interested in the results array
if (!empty($filename)) {
    $results = json_decode(
    ...
    // initialize units to assign units dinamically
    $units = [
        'pm25' => null,
        'pm10' => null
    ];
    
    // cycle over the results array and assign units based on units used in the results array
    ...
    // cycle over the results array, we extrapolate the months of the results and populate the stats array
    $stats = [];
    foreach ($results AS $result) {
        // var_dump($result);
        ...
        // populate arrays of data based on month and filter parameter
...
<?php else: ?>
    ...
    <?php if (!empty($stats)): ?>
        ...
        <?php
            ...
            // create and populate array of values of misurations
            $pm25 = [];
            $pm10 = [];
